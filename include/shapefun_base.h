...
public:
  ...
  virtual void evaluate_shape_fun(const std::array<double, Tdim> &xi, Eigen::Matrix<double, 1, numFun> &sFun) = 0;

  //! Evaluate gradient of shape functions
  //! \param[in] xi given local coordinates
  //! \param[out] gradSfun local gradients of shape functions
  virtual void evaluate_grad_shape_fun(const std::array<double, Tdim> &xi, Eigen::Matrix<double, Tdim, numFun> &gradSfun) = 0;
  ...
  template<unsigned p>
  virtual void quadrature_points(Eigen::Matrix<double, Tdim, p> &qpoints) = 0;
