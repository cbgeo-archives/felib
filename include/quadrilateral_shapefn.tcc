...
inline Eigen::Matrix<double, 9, 1> felib::QuadrilateralShapeFn<2, 9>::shapefn(
  ...
  shapefn_(4) = -0.5 * xi.at(1) * (xi.at(1) - 1.) * ((xi.at(0) * xi.at(0)) - 1.);
  shapefn_(5) = -0.5 * xi.at(0) * (xi.at(0) + 1.) * ((xi.at(1) * xi.at(1)) - 1.);
  shapefn_(6) = -0.5 * xi.at(1) * (xi.at(1) + 1.) * ((xi.at(0) * xi.at(0)) - 1.);
  shapefn_(7) = -0.5 * xi.at(0) * (xi.at(0) - 1.) * ((xi.at(1) * xi.at(1)) - 1.);
...
inline Eigen::Matrix<double, 9, 2> felib::QuadrilateralShapeFn<
  ...
  grad_shapefn_(5, 0) = -0.5 * (2. * xi.at(0) + 1.) * ((xi.at(1) * xi.at(1)) - 1.);
  grad_shapefn_(6, 0) = -xi.at(0) * xi.at(1) * (xi.at(1) + 1.);
  grad_shapefn_(7, 0) = -0.5 * (2. * xi.at(0) - 1.) * ((xi.at(1) * xi.at(1)) - 1.);
  grad_shapefn_(8, 0) = 2. * xi.at(0) * ((xi.at(1) * xi.at(1)) - 1.);
  grad_shapefn_(0, 1) = 0.25 * xi.at(0) * (xi.at(0) - 1.) * (2. * xi.at(1) - 1.);
  grad_shapefn_(1, 1) = 0.25 * xi.at(0) * (xi.at(0) + 1.) * (2. * xi.at(1) - 1.);
  grad_shapefn_(2, 1) = 0.25 * xi.at(0) * (xi.at(0) + 1.) * (2. * xi.at(1) + 1.);
  grad_shapefn_(3, 1) = 0.25 * xi.at(0) * (xi.at(0) - 1.) * (2. * xi.at(1) + 1.);
  grad_shapefn_(4, 1) = -0.5 * (2. * xi.at(1) - 1.) * ((xi.at(0) * xi.at(0)) - 1.);
  grad_shapefn_(5, 1) = -xi.at(0) * xi.at(1) * (xi.at(0) + 1.);
  grad_shapefn_(6, 1) = -0.5 * (2. * xi.at(1) + 1.) * ((xi.at(0) * xi.at(0)) - 1.);
