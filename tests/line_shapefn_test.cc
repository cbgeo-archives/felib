...
TEST_CASE("Line shape functions are checked", "[linesf][line][shapefn][1D]") {
  ...
  SECTION("Line shape function with two nodes") {
    ...
    // Coordinates is (-1);
    SECTION("Two noded line shape function for coordinates(-1)") {
      std::array<double, Dim> coords = {{ -1. }};
      auto shapefn = linesf->shapefn(coords);
    ...
    // Coordinates is (1)
    SECTION("Two noded line shape function for coordinates(1,1)") {
      std::array<double, Dim> coords = {{ 1. }};
      auto shapefn = linesf->shapefn(coords);
  ...
  SECTION("Line shape function with three nodes") {
    ...
    // Coordinates is (-1);
    SECTION("Three noded line shape function for coordinates(-1)") {
      std::array<double, Dim> coords = {{ -1. }};
      auto shapefn = linesf->shapefn(coords);
    ...
    // Coordinates is (1)
    SECTION("Three noded line shape function for coordinates(1,1)") {
      std::array<double, Dim> coords = {{ 1. }};
      auto shapefn = linesf->shapefn(coords);
